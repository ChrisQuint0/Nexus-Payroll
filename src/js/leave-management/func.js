...
async function handleAddLeave(event) {
  ...
  try {
    ...
    try {
      ...
      const leaveEndFormatted = leaveEnd.toISOString().split("T")[0];
      const description = `Added leave record: ${employeeName} - ${leaveType} (${duration} day(s), ${paidStatus}) from ${startDate} to ${leaveEndFormatted}. Balance updated: ${currentBalance} → ${newBalance}`;
