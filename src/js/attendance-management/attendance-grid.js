...
async function saveEditedRow(rowData, fieldChanged) {
  try {
    ...
    if (!data || data.length === 0) {
      console.warn(
        "⚠️ No records updated. Check if employee ID and date match."
      );
      alert(
...
async function generateCSVFile(data, cutoffFilter, departmentFilter) {
  ...
  const viewName =
    currentView === "raw" ? "Raw_Time_Logs" : "Attendance_Summary";
  const cutoffLabel =
    cutoffFilter === "all"
      ? "All_Cutoff"
...
async function generatePDFFile(data, cutoffFilter, departmentFilter) {
  try {
