...
async function loadDepartments() {
  try {
    ...
    if (departments.length > 0) {
      ...
      deptSelect.innerHTML =
        '<option value="">All Departments</option><option value="" disabled>No departments found</option>';
    }
  } catch (error) {
    ...
    if (deptSelect) {
      deptSelect.innerHTML =
        '<option value="">All Departments</option><option value="" disabled>Error loading departments</option>';
    }
  }
}
...
async function handleGenerateDTRClick(e) {
  ...
  try {
    ...
    if (selectedDepartment && selectedDepartment !== "") {
      ...
      try {
        dtrInfoArray = await generateDepartmentDTR(
          selectedDepartment,
          cutoffPeriod
        );
      } catch (error) {
        ...
        showError(
          ` Failed to generate DTR for ${selectedDepartment}. Check console for details.`
        );
        return;
      }
      ...
      if (!gridApi || typeof gridApi.getSelectedRows !== "function") {
        alert(
          " Grid API not available. Please select a department or refresh the page."
        );
        return;
      }
      ...
      if (selectedRows.length === 0) {
        alert(
          " Please select at least one employee from the grid, or choose a specific department"
        );
        return;
      }
      ...
      for (const row of selectedRows) {
        ...
        const employeeName = `${row["First Name"] || ""} ${
          row["Last Name"] || ""
        }`.trim();
        ...
        try {
          ...
          console.error(
            ` Failed to generate DTR for ${employeeName} (${employeeId}):`,
            error
          );
          errorCount++;
        }
    ...
    if (dtrInfoArray.length > 0) {
      try {
        ...
        showSuccess(
          ` Successfully generated ${dtrInfoArray.length} DTR(s) in PDF`
        );
      } catch (error) {
...
async function loadCutoffPeriodsForSelectedEmployee() {
  try {
    ...
    if (gridApi && typeof gridApi.getSelectedRows === "function") {
      ...
      if (selectedRows.length > 0) {
        employeeId =
          selectedRows[0]["Employee ID"] || selectedRows[0].employee_id;
      }
    }
...
function showLoading(message) {
  ...
  toast.className =
    "alert alert-info fixed bottom-4 right-4 w-auto shadow-lg z-50";
  toast.innerHTML = `
...
function showSuccess(message) {
  const toast = document.createElement("div");
  toast.className =
    "alert alert-success fixed bottom-4 right-4 w-auto shadow-lg z-50";
  toast.innerHTML = `
...
function showWarning(message) {
  const toast = document.createElement("div");
  toast.className =
    "alert alert-warning fixed bottom-4 right-4 w-auto shadow-lg z-50";
  toast.innerHTML = `
    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
function showError(message) {
  const toast = document.createElement("div");
  toast.className =
    "alert alert-error fixed bottom-4 right-4 w-auto shadow-lg z-50";
  toast.innerHTML = `
    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
