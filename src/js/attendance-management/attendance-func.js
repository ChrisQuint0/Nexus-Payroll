...
import { gridApi, switchView, applyDataFilter, getCurrentData, getCurrentView } from './attendance-grid.js';
...
rawTimeLogsTab.addEventListener('click', function() {
  ...
  rawTimeLogsTab.className = 'px-5 py-2 rounded-lg text-sm font-semibold btn btn-primary';
  attendanceSummaryTab.className = 'px-5 py-2 rounded-lg text-sm font-semibold btn btn-outline';
...
attendanceSummaryTab.addEventListener('click', function() {
  ...
  attendanceSummaryTab.className = 'px-5 py-2 rounded-lg text-sm font-semibold btn btn-primary';
  rawTimeLogsTab.className = 'px-5 py-2 rounded-lg text-sm font-semibold btn btn-outline';
    ...
    if (selectedRows.length > 0) {
      ...
      selectedRows.forEach((row, index) => {
        try {
          ...
          errorCount++;
          alert(`Error generating DTR for Employee ${row["Employee ID"]}: ${error.message}`);
        }
      });
      
      if (successCount > 0) {
        alert(` Generated DTR for ${successCount} employee(s)\nCutoff: ${cutoffPeriod}\n\n${errorCount > 0 ? ` ${errorCount} failed` : 'Check your browser for print windows.'}`);
      }
    } else {
      alert(' Please select at least one employee to generate DTR.\n\n Use the checkboxes in the grid to select employees.');
    }
...
if (generateCSVForm) {
  generateCSVForm.addEventListener('click', function(e) {
    ...
    console.log('Generating CSV for:', { cutoffPeriod, department, view: currentView });
    ...
    if (filteredData.length > 0) {
      ...
      alert(` Generated CSV for ${filteredData.length} rows\nView: ${currentView === 'raw' ? 'Raw Time Logs' : 'Attendance Summary'}\nCutoff: ${cutoffPeriod}\nDepartment: ${department}`);
    } else {
...
function exportToCSV(data, cutoffPeriod, department, currentView) {
  ...
  const headers = Object.keys(data[0]).filter(header => 
    header !== 'Cutoff Period' // Exclude cutoff period from CSV export
  );
  ...
  const filename = `${currentView === 'raw' ? 'Time_Logs' : 'Attendance_Summary'}_${cutoffPeriod.replace(/\s/g, '_')}_${department.replace(/\s/g, '_')}.csv`;
