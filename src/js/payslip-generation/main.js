...
dummyEmployees.forEach(emp => {
    ...
    emp.totalDeductions = emp.sss + emp.philhealth + emp.pagibig + emp.withholdingTax;
...
const columnDefs = [
    ...
    },
    {
        ...
        cellRenderer: (params) => {
            ...
            button.className = 'px-8 py-2 rounded-lg font-medium text-white transition-colors';
...
try {
    ...
    alert('Error loading the grid. Please make sure AG Grid library is loaded correctly.');
}
...
function generateEmployeesPdf(employees) {
    ...
    employees.forEach((emp, index) => {
        console.log('[Payslip] Rendering panel for employee:', emp && emp.employeeId, emp && emp.name);
        ...
        doc.roundedRect(panelLeft, panelTop, panelWidth, estimatedPanelHeight, 6, 6, 'S');
        ...
        doc.text(`${emp.department} • ${emp.position}`, panelRight - innerPadding, yy, { align: 'right' });
        ...
        drawLabelValue('Regular Pay', formatCurrency(emp.regularPay), col1X, yy, colWidth - valueRightOffset);
        yy += 14;
        drawLabelValue('OT', formatCurrency(emp.overtimePay), col1X, yy, colWidth - valueRightOffset);
        yy += 14;
        drawLabelValue('Leave Pay', formatCurrency(emp.leavePay), col1X, yy, colWidth - valueRightOffset);
        yy += 16;
        drawLabelValue('Working Hours', formatCurrency(emp.workingHours), col1X, yy, colWidth - valueRightOffset);
        yy += 14;
        drawLabelValue('Absences', formatCurrency(emp.absences), col1X, yy, colWidth - valueRightOffset);
        yy += 14;
        drawLabelValue('Tardiness', formatCurrency(emp.tardiness), col1X, yy, colWidth - valueRightOffset);
        yy += 14;
        drawLabelValue('Undertime', formatCurrency(emp.undertime), col1X, yy, colWidth - valueRightOffset);

        // Right column: contributions + summary
        let yy2 = panelTop + innerPadding + 22;
        drawLabelValue('SSS Contribution', formatCurrency(emp.sss), col2X, yy2, colWidth - valueRightOffset);
        yy2 += 14;
        drawLabelValue('PhilHealth', formatCurrency(emp.philhealth), col2X, yy2, colWidth - valueRightOffset);
        yy2 += 14;
        drawLabelValue('Pag-ibig', formatCurrency(emp.pagibig), col2X, yy2, colWidth - valueRightOffset);
        yy2 += 14;
        drawLabelValue('Withholding Tax', formatCurrency(emp.withholdingTax), col2X, yy2, colWidth - valueRightOffset);
        yy2 += 16;
        drawLabelValue('Gross Earnings', formatCurrency(emp.grossEarnings), col2X, yy2, colWidth - valueRightOffset);
        yy2 += 14;
        drawLabelValue('Total Deductions', formatCurrency(emp.totalDeductions), col2X, yy2, colWidth - valueRightOffset);
        yy2 += 18;
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(15, 81, 50); // green-ish
        drawLabelValue('Net Pay', formatCurrency(emp.netPay), col2X, yy2, colWidth - valueRightOffset);
...
if (generatePdfBtn) {
    generatePdfBtn.addEventListener('click', () => {
        ...
        if (!gridApi) {
            console.warn('[Payslip] gridApi is not set. Falling back to all dummy employees.');
        }
...
function openPreviewModal(employeeId) {
    ...
    const dates = periodMap[employee.cutoffPeriod] || { period: '09/01/25 - 09/15/25', issued: '09/16/25' };
